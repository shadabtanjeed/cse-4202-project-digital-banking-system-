...
void CreditCardManagement(char *username, int count, CreditCardInfo *credit_card_info);
...
void ApplyForCreditCard(char *username, int count, CreditCardInfo *credit_card_info)
{
    ...
    credit_card_info[count].ActiveStatus = 0;
    ...
    strcpy(credit_card_info[count].Pin, pin);
    ...
    {
        fprintf(fp, "Card No: %s\nCard Type: %s\nExpiry Date: %s\nCVV: %s\nCard Limit: %lld\nAvailable Balance: %lld\nUsername: %s\nPin: %s\nActive Status: %d\nRank: %d\n", credit_card_info[i].CardNo, credit_card_info[i].CardType, credit_card_info[i].ExpiryDate, credit_card_info[i].CVV, &credit_card_info[i].CardLimit, &credit_card_info[i].AvailableBalance, credit_card_info[i].Username, credit_card_info[i].Pin, credit_card_info[i].ActiveStatus, credit_card_info[i].Rank);
    }
    fclose(fp);
}
...
void CloseAccount(char *username, int count, int count_user, AccountInfo *account_info, Username *user_name)
{
    ...
    else
    {
        ...
        scanf("%d", &AccChoice);
        int index = matchingAccounts[AccChoice - 1];
